...
const api: ElectronApi = {
  ...
  ipcExec: async (exeFile, args) =>
    await ipcRenderer.invoke("exec-command", exeFile, args),
  openBinDir: async () => {
  ...
  addVideo: async (video: Video) =>
    await ipcRenderer.invoke("add-video", video),
  updateVideo: async (id: string, video: Partial<Video>) =>
    await ipcRenderer.invoke("update-video", id, video),
  removeVideo: async (id?: string) =>
    await ipcRenderer.invoke("remove-video", id),
  getCollectionList: async () =>
    await ipcRenderer.invoke("get-collection-list"),
  addCollection: async (video: Video) =>
    await ipcRenderer.invoke("add-collection", video),
