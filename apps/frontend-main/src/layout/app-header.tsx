...
export function AppHeader({ className }: Props) {
  ...
  return (
    <div
      className={cn("flex h-16 w-full select-none flex-row justify-between bg-[#F9FBFC] dark:bg-[#1F2024]", className)}
    >
      <div className="h-full rounded-br-full bg-[#EBF3FB] pr-2 dark:bg-[#3B3C41]">
        <div className="relative flex h-full min-w-[299px] flex-row items-center rounded-br-full bg-[#fff] pl-3 pr-2 dark:bg-[#2C2E33]">
          ...
          <span className="ml-[30px] block text-sm text-[#666] dark:text-white">v{import.meta.env.APP_VERSION}</span>
      ...
      <div className="flex flex-row items-center gap-3 pr-3">
        <div className="cursor-pointer rounded-full rounded-br-sm bg-white p-1 dark:bg-[#43454B]" onClick={openHelpUrl}>
          <div className="flex flex-row items-center gap-2 rounded-full rounded-br-sm bg-[#F9FBFC] p-2 dark:bg-[#2F3035]">
            ...
            <span className="text-sm text-[#137BF4] dark:text-white">{t("help")}</span>
          </div>
        </div>
      </div>
    </div>
