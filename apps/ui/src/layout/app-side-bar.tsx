...
import { cloneElement, type PropsWithChildren, type ReactElement, useMemo } from "react";
import { useTranslation } from "react-i18next";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { useShallow } from "zustand/react/shallow";
import siderBg from "@/assets/images/sider-bg.png";
import { ConverterIcon, DoneIcon, ExtractIcon, ListIcon, SettingsIcon, ShareIcon } from "@/assets/svg";
...
function AppMenuItem({ children, onClick, link, activeKey, className, icon }: AppMenuItemProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        {
          "bg-gradient-to-r from-[#127AF3] to-[#06D5FB] text-white hover:text-white dark:text-white": isActive,
        },
        className,
      )}
      onClick={onClick}
...
export function AppSideBar({ className }: Props) {
  ...
  const { count, clearCount } = useDownloadStore(useShallow(downloadStoreSelector));
  const appStore = useAppStore(useShallow(appStoreSelector));
  const { setAppStore } = useAppStore(useShallow(setAppStoreSelector));
  const { updateAvailable } = useSessionStore(useShallow(updateSelector));

  const activeKey = useMemo(() => processLocation(location.pathname), [location.pathname]);

  const handleExternalLink = useMemoizedFn(async (e: React.MouseEvent<HTMLDivElement>) => {
    e.stopPropagation();
    e.preventDefault();
  ...
  const items1: MenuItem[] = useMemo(() => {
    return [
      {
        label: (
          <AppMenuItem link="/" onClick={handleClearCount} activeKey={activeKey} icon={<ListIcon />}>
            <span>{t("downloadList")}</span>
            {count > 0 && <Badge count={count} offset={[5, 1]} size="small"></Badge>}
          </AppMenuItem>
        ),
        key: "home",
      },
      ...
      {
        label: (
          <AppMenuItem link="/converter" activeKey={activeKey} icon={<ConverterIcon />}>
      ...
      {
        label: (
          <AppMenuItem link="/source" activeKey={activeKey} className="group" icon={<ExtractIcon />}>
            ...
            >
              <ShareIcon fill={"/source" === location.pathname ? "#fff" : "#AAB5CB"} />
            </div>
          </AppMenuItem>
        ),
        key: "source",
      ...
      {
        label: (
          <AppMenuItem link="/settings" activeKey={activeKey} icon={<SettingsIcon />}>
            <span>{t("setting")}</span>
  ...
  }, [activeKey, count, handleExternalLink, location.pathname, t, updateAvailable, handleClearCount]);

  const finalItems = useMemo(() => {
    return items1
      .filter((i) => (isWeb ? i.key !== "source" && i.key !== "converter" : true))
      .filter((item) => (appStore.openInNewWindow ? item?.key !== "source" : true));
  }, [items1, appStore.openInNewWindow]);

  return (
    <div className={cn("relative select-none bg-[#fff] p-3 dark:bg-[#1F2024]", className)}>
      ...
      <img src={siderBg} alt="" className="pointer-events-none absolute bottom-0 left-0 right-0 w-full select-none" />
    </div>
  );
}
