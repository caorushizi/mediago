...
import { App, AutoComplete, Button, Form, Input, Modal, Select, Switch } from "antd";
...
export default forwardRef<DownloadFormRef, DownloadFormProps>(function DownloadForm(
  { isEdit, destroyOnClose, onAddToList, onDownloadNow, onAddToDocker, usePrevData, onFormVisibleChange, id },
  ref,
) {
  ...
  return (
    <Modal
      ...
      footer={[
        ...
        enableDocker && (
          <Button key="docker" onClick={handleAddToDocker} icon={<DockerOutlined />}>
            {t("addToDocker")}
          </Button>
        ),
        <Button key="submit" onClick={handleSubmit}>
          {isEdit && !usePrevData ? t("confirmChange") : t("addToDownloadList")}
        </Button>,
    ...
    >
      ...
      >
                  ...
                  },
                  {
                    validator: (_, value) => {
                      ...
                      for (const line of lines) {
                        ...
                        if (params.length > 3) {
                          return Promise.reject(new Error(t("pleaseEnterCorrectBatchList")));
                        }
                        const [url] = params;
                        if (!/^(https?):\/\/.+/.test(url)) {
                          return Promise.reject(new Error(t("pleaseEnterCorrectBatchList")));
        ...
        <Form.Item noStyle shouldUpdate>
          {(form) => {
            if (form.getFieldValue("type") !== "m3u8" && !form.getFieldValue("batch")) {
              return null;
            }
            return (
              <Form.Item label={t("additionalHeaders")} name="headers">
                <TextArea rows={4} placeholder={t("additionalHeadersDescription")} />
              </Form.Item>
            );
          }}
        </Form.Item>
